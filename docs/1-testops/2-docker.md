...
- Delete all stopped containers:
    ...
    * `docker rm | xargs docker ps -aq -f status=exited`  
...
- Build an image using Dockerfile: `docker build -t qmeta/myapp:1.0 .`
...
LABEL maintainer="patrick@mail.com"
...
**FROM** : Docker的Base Image

`FROM <image>[:<tag>|@<digest>]`

`FROM ubuntu:latest`

**COPY**: 从本地复制到docker image中
